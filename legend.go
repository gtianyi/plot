...
var legendEntryIsBetterThan func(a,b string)bool

type legendEntries []legendEntry

func (a legendEntries) Len() int {
	return len(a)
}

func (a legendEntries) Less(i, j int) bool {
	return legendEntryIsBetterThan(a[i].text, a[j].text)
}

func (a legendEntries) Swap(i, j int) {
	a[i], a[j] = a[j], a[i]
}

func (l *Legend) SortLegend(isBetterThan func(a,b string)bool) {
	legendEntryIsBetterThan = isBetterThan
	sort.Sort(legendEntries(l.entries))
}
